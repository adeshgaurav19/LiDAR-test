...
def strategy_for_easy(df: pd.DataFrame):
    ...
    for i in range(len(steps) - 1):
        ...
        s_clusters = [{'c': np.mean(s_pts[labels == l], axis=0), 'p': s_pts[labels == l]} 
...
def visualize_results(df: pd.DataFrame, clusters: list, difficulty: str, output_dir: str):
    """Create and save visualization of clustering results."""
    fig = plt.figure(figsize=(12, 9))
    ax = fig.add_subplot(111, projection='3d')
    
    # Plot original data in background
    ax.scatter(df['x'], df['y'], df['z'], c='lightgray', s=1, alpha=0.1, label='Original Data')
    
    # Plot clustered wires
    colors = ['red', 'blue', 'green', 'orange', 'purple', 'brown', 'pink']
    for i, wire_cloud in enumerate(clusters):
        color = colors[i % len(colors)]
        ax.scatter(wire_cloud[:, 0], wire_cloud[:, 1], wire_cloud[:, 2], 
                  c=color, s=5, label=f'Wire {i+1} ({len(wire_cloud)} pts)')
    
    ax.set_title(f"Wire Detection Results - '{difficulty.upper()}' Dataset", fontsize=14)
