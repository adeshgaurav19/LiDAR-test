...
from catenary_fitting import (
    fit_catenary_to_wire_clusters,
...
def run_end_to_end_pipeline(run_eda=True):
    ...
    for difficulty, df in lidar_dataframes.items():
        ...
        if not wire_clusters or not all(len(c) > 0 for c in wire_clusters):
            print(f"❌ Clustering failed for '{difficulty}'")
            continue
        ...
        if not catenary_results["wire_params"]:
            print(f"❌ Catenary fitting failed for '{difficulty}'")
            continue
        ...
        visualize_catenary_results(
            ...
            wire_clusters,
            catenary_results,
            f"Final Model for {difficulty.upper()}",
            plot_path,
        )
