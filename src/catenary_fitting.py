...
from clustering import strategy_for_easy,strategy_for_medium_extrahard,strategy_hough_transform,align_point_cloud,reverse_alignment
...
def save_results_to_json(data: dict, output_path: str):
    ...
    for difficulty, results in data.items():
        ...
        converted_results['wire_clusters'] = [w.tolist() for w in results['wire_clusters']]
...
def visualize_catenary_results(original_df, wire_clusters, catenary_results, title="Catenary Fitting Results", output_dir=None, difficulty="unknown"):
    """
    ...
    ax1.scatter(original_df['x'], original_df['y'], original_df['z'], c='lightgray', s=1, alpha=0.3, label='Original Points')
    ...
    for i, (wire_points, curve_3d, fit_quality) in enumerate(zip(
        ...
        ax1.scatter(wire_points[:, 0], wire_points[:, 1], wire_points[:, 2], c=color, s=10, alpha=0.7)
        ax1.plot(curve_3d[:, 0], curve_3d[:, 1], curve_3d[:, 2], c=color, linewidth=3, linestyle='--', label=f'Wire {i+1} Catenary (R²={fit_quality:.2f})')
    ax1.set_title(f'{title} - 3D View'); ax1.legend()
    ...
    ax2.set_title(f'{title} - X-Z Projection'); ax2.grid(True, alpha=0.3)
...
def run_complete_analysis_with_catenary():
    ...
    for difficulty, df in lidar_dataframes.items():
        ...
        if not wire_clusters or not all(len(c) > 0 for c in wire_clusters): continue
        ...
        if not catenary_results['wire_params']: continue
        
        # --- Visualize and Save Plot ---
        plot_path = os.path.join(plot_output_dir, f"{difficulty}_final_visualization.png")
        visualize_catenary_results(df, wire_clusters, catenary_results, f"{difficulty.upper()} Dataset", plot_path)
